    ...
    public RedirectView getTiny(@PathVariable String tiny) throws JsonProcessingException {
        ...
        if (tinyRequest.getLongUrl() != null) {
            ...
            if ( userName != null) {
                ...
                userClickRepository.save(anUserClick().userClickKey(anUserClickKey().withUserName(userName).withClickTime(new Date()).build()).tiny(tiny).longUrl(tinyRequest.getLongUrl()).build());

            }
            System.out.println(tinyRequest.getLongUrl());
            RedirectView redirectView = new RedirectView();
            redirectView.setUrl("https://"+tinyRequest.getLongUrl());
            return redirectView;
        } else {
